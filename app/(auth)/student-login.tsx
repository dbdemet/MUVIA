...
import { getUserProfile, loginStudent } from '../../services/api';
...
export default function StudentLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (!e.endsWith('@posta.mu.edu.tr')) { setError('Student email must end with @posta.mu.edu.tr'); return; }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={st.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={st.header}>
            ...
            <Text style={st.title}>Student Login</Text>
            <Text style={st.subtitle}>Sign in with your MSKU account</Text>
          </View>
          <View style={st.form}>
            <Text style={st.label}>University Email</Text>
            <View style={st.inputWrap}>
              <Text style={st.inputIcon}>📧</Text>
              <TextInput style={st.input} placeholder="username@posta.mu.edu.tr" placeholderTextColor="#A0AEC0" value={email} onChangeText={v => { setEmail(v); if (error) setError(''); }} keyboardType="email-address" autoCapitalize="none" />
            </View>
            ...
            <TouchableOpacity style={st.forgotBtn} onPress={() => {
              ...
              if (!email.trim().toLowerCase().endsWith('@posta.mu.edu.tr')) { Alert.alert('Invalid Email', 'Enter a valid @posta.mu.edu.tr email.'); return; }
            ...
            <View style={st.infoBox}><Text style={st.infoText}>🔒  Use your OBS credentials to sign in.{'\n'}Your data is protected by MSKU security standards.</Text></View>
          </View>
        </ScrollView>
...
const st = StyleSheet.create({
  ...
  iconWrap: { width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(42,105,172,0.25)', alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  icon: { fontSize: 40 },
  ...
  forgotText: { color: '#2A69AC', fontSize: 13, fontWeight: '600' },
  loginBtn: { backgroundColor: '#1A365D', borderRadius: 14, padding: 16, alignItems: 'center', marginTop: 16 },
  loginBtnDisabled: { backgroundColor: '#A0AEC0' },
  loginBtnText: { color: '#FFFFFF', fontSize: 16, fontWeight: '700', letterSpacing: 0.5 },
  infoBox: { backgroundColor: '#EBF4FF', borderRadius: 12, padding: 14, marginTop: 16 },
  infoText: { color: '#2A69AC', fontSize: 12, lineHeight: 20 },
});